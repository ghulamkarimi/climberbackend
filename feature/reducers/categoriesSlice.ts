...
export const editCategoriesApi = createAsyncThunk(
  "/categories/editCategoriesApi",
  async ({
    categoryId,
    category,
  }: {
    ...
    try {
      const response = await editCategories(categoryId, category);
      return response.data;
    } catch (error: any) {
      throw error.response.data.message;
    }
...
const categoriesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(editCategoriesApi.fulfilled,(state,action)=>{
        categoriesAdapter.updateOne(state,action.payload.category._id)
    })
  },
});
